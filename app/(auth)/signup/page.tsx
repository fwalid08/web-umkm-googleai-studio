  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12">
      <div className="max-w-md w-full space-y-6">
        ...
        <button onClick={()=> signIn("google", { callbackUrl: "/websites" })} className="w-full flex items-center justify-center gap-2 py-3 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 font-medium">
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div><label className="block text-sm font-medium mb-1">{t("onboarding.s2Title")}</label><select {...register("business_type")} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-green-500">{bizOpts.map(o=> <option key={o.v} value={o.v}>{o.e} {o.label}</option>)}</select></div>
